...
来源：https://github.com/Colin-Fredericks/hx_util
20220414 linklist.extend() 改为 linklist.extend(newlinks)
"""
...
def writeFile(linklist, filecount, outFileName, outFilePath, args):
    ...
    # 修复乱码问题
    df = pd.read_csv(outFilePath, encoding='utf-8')
    df.to_csv(outFilePath, encoding="utf_8_sig")
    # print("Spreadsheet created: " + outFileName)
    print("Location: " + outFilePath)


def getExcelLinks(args, args_path):
    ...
    if args.l:
        ...
        if target_is_folder:
            outFilePath = os.path.join(args_path, outFileName)
            ...
            outFilePath = os.path.join(args_path, outFileName) # 修改输出文件路径
...
if __name__ == "__main__":
    ...
    getExcelLinks(args, args_path) # 此处改为一个列表来传入